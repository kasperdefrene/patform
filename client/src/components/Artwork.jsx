const Artwork = ({radius, colorOne, colorTwo, rotation, offsetOne, offsetTwo, xPos, yPos, showStars}) => {
    ...
    const stars = () => {
        let stars = [];
        for (let i = 0; i < 100; i++) {
          stars.push(<circle cx={Math.random() * 100 + '%'} cy={Math.random() * 100 + '%'} r="1" fill="grey" key={i} />);
        }
        return stars;
    };

    return (
        <svg viewBox="0 0 118 255">
            {showStars && stars()}

            <defs>
                <circle id="sun" cx="250" cy="250" r={radius}></circle>

                <linearGradient id="myGradient" gradientTransform={rotation}>
                    <stop offset={offsetOne} stopColor={colorOne} />
            ...
            <svg x={xPos} y={yPos} width="50" height="50" viewBox="0 0 642 642" fill="none" xmlns="http://www.w3.org/2000/svg">
